...
FIXTURE_PATH = (
    Path(__file__).resolve().parents[2] / "fixtures" / "matlab_reference" / "dx2abc_fixture.mat"
)
...
@pytest.mark.parity
def test_dx2abc_parity_baseline_and_stable1_fixture() -> None:
    if not FIXTURE_PATH.exists():
        pytest.skip(
            "Missing MATLAB fixture. Generate with: testutils.generateDx2abcFixture in MATLAB."
        )
    ...
    if n_cases < 3:
        pytest.skip(
            "Fixture has fewer than 3 cases. Regenerate with: testutils.generateDx2abcFixture"
        )
    ...
    for i in range(n_cases):
        ...
        a, b, c = dx2abc(x_base, u_base, y_base, f_base, p_base, c="none")
        ...
        a1, b1, c1 = dx2abc(x_s1, u_s1, y_s1, f_s1, p_s1, c="stable1")
    ...
    write_parity_report("dx2abc_parity_report", rows)