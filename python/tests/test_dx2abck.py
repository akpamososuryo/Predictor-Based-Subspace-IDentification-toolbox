...
def _make_consistent_dataset(
    *,
    ...
    c = rng.standard_normal((l_out, n)) * 0.4
    d = rng.standard_normal((l_out, r)) * 0.05
    k = rng.standard_normal((n, l_out)) * 0.08

    u = rng.standard_normal((r, n_samples))
    e = 0.02 * rng.standard_normal((l_out, n_samples))
...
def test_dx2abck_baseline_shapes_and_finite() -> None:
    x, u, y, f, p = _make_consistent_dataset()

    a, b, c = dx2abck(x, u.T, y.T, f, p)
...
def test_dx2abck_return_k_shapes_and_finite() -> None:
    x, u, y, f, p = _make_consistent_dataset(seed=456)
    a, b, c, k = dx2abck(x, u, y, f, p, return_k=True)
...
def test_dx2abck_rejects_invalid_windows() -> None:
    x, u, y, _, p = _make_consistent_dataset()
    with pytest.raises(ValueError, match="f <= p"):
        _ = dx2abck(x, u, y, f=p + 1, p=p)


def test_dx2abck_stable1_enforces_stability() -> None:
    x, u, y, f, p = _make_unstable_dataset()
    a, b, c = dx2abck(x, u, y, f, p, c="stable1")
...
def test_dx2abck_stable2_enforces_stability() -> None:
    x, u, y, f, p = _make_unstable_dataset(seed=654)
    a0, _, _ = dx2abck(x, u, y, f, p, c="none")
    a, b, c = dx2abck(x, u, y, f, p, c="stable2")
