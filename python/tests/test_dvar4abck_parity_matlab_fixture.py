...
FIXTURE_PATH = (
    Path(__file__).resolve().parents[2] / "fixtures" / "matlab_reference" / "dvar4abck_fixture.mat"
)
...
@pytest.mark.parity
def test_dvar4abck_parity_base_and_stable1_fixture() -> None:
    if not FIXTURE_PATH.exists():
        pytest.skip(
            "Missing MATLAB fixture. Generate with: testutils.generateDvar4abckFixture in MATLAB."
        )
    ...
    if n_cases < 3:
        pytest.skip(
            "Fixture has fewer than 3 cases. Regenerate with: testutils.generateDvar4abckFixture"
        )
    ...
    for i in range(n_cases):
        ...
        p_cov_b, sigma_b, da_b, db_b, dc_b, dk_b = base_result
        ...
        p_cov_s1, sigma_s1, da_s1, db_s1, dc_s1, dk_s1 = stable_result
    ...
    write_parity_report("dvar4abck_parity_report", rows)